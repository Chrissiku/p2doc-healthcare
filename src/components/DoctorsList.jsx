  ...
  return (
    <div className="mx-auto h-screen bg-white w-[85%] p-3 rounded-lg">
      ...
      <div className="grid grid-cols-2 gap-9 justify-center items-center px-5 xl:grid-cols-4 ">
        {paginatedList.map((doctor, index) => (
          ...
          >
            ...
            <span>
              {doctor.did
                ? doctor.did.slice(8, 20) + "..." + doctor.did.slice(-8)
                : ""}
            </span>
      ...
      <div className="flex justify-center items-center gap-10 mt-10">
        ...
        <button
          onClick={() => setCurrentPage(currentPage + 1)}
          disabled={currentPage === Math.ceil(doctorList.length / itemsPerPage)}
          className={`${
            currentPage === Math.ceil(doctorList.length / itemsPerPage)
              ? "text-gray-400"
              : "text-black"
          }`}
        >
